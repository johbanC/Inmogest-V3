...
<div class="card">
    <div class="card-header">
        ...
        <div class="card-tools">
            ...
            @include('calentadors.modal.modal-nuevo')
    ...
    <div class="card-body">        
        ...
        <table id="example1" head-theme="light" theme="light" class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $calentador->nombre }}</td>
                    <td>
                        <button class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar" data-toggle="modal" data-target="#modalEditar{{ $calentador->id }}">
                            <i class="fa fa-lg fa-fw fa-pen"></i>
                        </button>
                        <form id="formDelete{{ $calentador->id }}" method="POST" action="{{ route('calentadors.destroy', $calentador->id) }}" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="button" onclick="eliminar({{ $calentador->id }})" class="btn btn-xs btn-default text-danger mx-1 shadow" title="Eliminar">
                ...
                @include('calentadors.modal.modal-edit')
                @endforeach
            </tbody>
        </table>
    </div>
